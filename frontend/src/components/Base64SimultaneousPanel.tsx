...
interface Base64SimultaneousPanelProps {
  onRemove: () => void;
}

export function Base64SimultaneousPanel({ onRemove }: Base64SimultaneousPanelProps) {
  ...
  useEffect(() => {
    ...
    // Decode
    try {
      const decoded = input ? atob(input) : '';
      setDecodedOutput(decoded);
    } catch (error) {
      setDecodedOutput(`Error: ${error instanceof Error ? error.message : 'Invalid base64 input'}`);
    }
  }, [input]);
      ...
      <CardHeader className="pb-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-lg">Base64 Encode/Decode Simultaneously</CardTitle>
          </div>
          <Button
            onClick={onRemove}
            variant="ghost"
      ...
      <CardContent className="pt-0">
        <div className="space-y-6">
          <div className="space-y-3">
            <Label htmlFor="base64-simultaneous-input" className="text-sm font-medium">Input</Label>
            <Textarea
              id="base64-simultaneous-input"
              value={input}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <Label htmlFor="base64-encoded-output" className="text-sm font-medium">Encoded Output</Label>
              <Textarea
                id="base64-encoded-output"
                value={encodedOutput}
                readOnly
                rows={6}
            ...
            <div className="space-y-3">
              <Label htmlFor="base64-decoded-output" className="text-sm font-medium">Decoded Output</Label>
              <Textarea

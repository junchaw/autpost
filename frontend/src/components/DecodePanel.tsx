...
export function DecodePanel({ onRemove }: DecodePanelProps) {
  ...
  useEffect(() => {
    try {
      ...
      setOutput(`Error: ${error instanceof Error ? error.message : 'Invalid base64 input'}`);
    }
  }, [input]);
      ...
      <CardContent className="pt-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <Label htmlFor="decode-input" className="text-sm font-medium">Input</Label>
            <Textarea
              id="decode-input"
              value={input}
          ...
          <div className="space-y-3">
            <Label htmlFor="decode-output" className="text-sm font-medium">Decoded Output</Label>
            <Textarea
              id="decode-output"
              value={output}
              readOnly

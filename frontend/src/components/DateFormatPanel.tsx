...
interface DateFormatPanelProps {
  onRemove: () => void;
}
...
const DOC_LINKS: Record<string, string> = {
  ...
  javascript: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat',
  c: 'https://en.cppreference.com/w/c/chrono/strftime',
};

export function DateFormatPanel({ onRemove }: DateFormatPanelProps) {
  ...
  useEffect(() => {
    // Set current timestamp on mount
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isNaN(date.getTime())) {
        setOutput('Error: Invalid date/time');
        return;
      }
      ...
      if (language === 'php') {
        formatted = formatPhp(date, format);
      } else if (language === 'python') {
        formatted = formatPython(date, format);
      } else if (language === 'golang') {
        formatted = formatGolang(date, format);
      } else if (language === 'javascript') {
        formatted = formatJavaScript(date, format);
      } else if (language === 'c') {
        formatted = formatC(date, format);
      }

      setOutput(formatted);
    } catch (err) {
      setOutput(`Error: ${err instanceof Error ? err.message : 'Invalid format'}`);
    }
      ...
      <CardContent className="pt-0">
        <div className="space-y-4">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                variant={language === 'php' ? 'default' : 'outline'}
                size="sm"
              >
                PHP
              </Button>
              ...
              <Button
                ...
                variant={language === 'golang' ? 'default' : 'outline'}
                size="sm"
              >
                Golang
              </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              <Label htmlFor="date-format-input" className="text-sm font-medium">Format String</Label>
              <Textarea
                id="date-format-input"
                value={format}
                onChange={(e) => setFormat(e.target.value)}
                placeholder={
                  language === 'php' ? 'Y-m-d H:i:s' :
                  language === 'python' ? '%Y-%m-%d %H:%M:%S' :
                  language === 'golang' ? '2006-01-02 15:04:05' :
                  language === 'javascript' ? 'YYYY-MM-DD HH:mm:ss' :
                  '%Y-%m-%d %H:%M:%S'
                }
                rows={3}
            ...
            <div className="space-y-3">
              <Label htmlFor="date-timestamp-input" className="text-sm font-medium">Date/Time (ISO 8601)</Label>
              <Textarea
          ...
          <div className="space-y-3">
            <Label htmlFor="date-format-output" className="text-sm font-medium">Formatted Output</Label>
            <Textarea
